...
class CapsNet:
	...
	def layer(	self,
		...
		if self.mode == 'conv2d':
			...
			inputs_expanded = tf.expand_dims(inputs, -1, name = 'inputs_expanded')
			inputs_tile     = tf.expand_dims(inputs_expanded,  2, name = 'inputs_tile')
	...
	def routing(self, inputs_tile, routing_logits, inputs_shape):
		...
		biases 			= tf.get_variable(name = 'biases', shape = (1, 1, self.caps_units, self.caps_dim, 1))
		...
		u_hat 			 	= tf.reduce_sum(Wu, axis = 3, keepdims = True, name = 'u_hat') # [1152, 160, 1, 1]
		pred_vec 	     	= tf.reshape(u_hat, shape = [-1, u_hat.shape[1].value, self.caps_units, self.caps_dim, 1], 
												name = 'pred_vec')
		pred_vec_stopped	= tf.stop_gradient(pred_vec, name = 'pred_vec_stopped')
		...
		for round_it in range(1, self.rounds + 1):
			with tf.variable_scope('round_' + str(round_it)):
				...
				if round_it == self.rounds:
					s_j = tf.multiply(coupling_coeff, pred_vec)
					s_j = tf.reduce_sum(s_j, axis = 1, keepdims = True) + biases
					v_j = self.squash(s_j)

				elif round_it < self.rounds:
					s_j = tf.multiply(routing_logits, pred_vec_stopped)
					s_j = tf.reduce_sum(s_j, axis = 1, keepdims = True) + biases
					v_j = self.squash(s_j)

					v_j_tiled   = tf.tile(v_j, [1, pred_vec.shape[1].value, 1, 1, 1])
					agreement = tf.reduce_sum(pred_vec_stopped * v_j_tiled, axis = 3, keepdims = True)
